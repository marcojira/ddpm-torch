...
class CustomDataset(tvds.ImageFolder):
    transform = transforms.Compose(
        [
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
...
def get_dataloader(
    ...
    **kwargs
    ...
    dataset_info = {"resolution": (32, 32), "channels": 3, "train_size": len(dataset)}
    if distributed:
        batch_size = batch_size // int(os.environ.get("WORLD_SIZE", "1"))
    dataset_kwargs = {"root": root, "split": split, "transform": dataset.transform}
